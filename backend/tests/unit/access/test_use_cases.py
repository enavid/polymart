...
and a fake channel repository, so the audit logging and slug->id resolution are
verified without guardian or the ORM.
"""
...
class TestAssignRole:
    def test_assigns_the_role_through_the_gateway(self, gateway: FakeAccessControlGateway) -> None:
        AssignRole(gateway).execute(user_id=7, role_name="channel_admin")

        assert gateway.roles == [(7, "channel_admin")]

    def test_records_the_acting_user_in_the_audit_event(
        self, gateway: FakeAccessControlGateway
    ) -> None:
        with capture_logs() as logs:
            AssignRole(gateway).execute(user_id=7, role_name="channel_admin", actor="root")
...
class TestGrantChannelManagement:
    def test_grants_object_scope_for_the_resolved_channel(
        self, gateway: FakeAccessControlGateway, channels: FakeChannelRepository
    ) -> None:
        GrantChannelManagement(gateway, channels).execute(user_id=7, channel_slug="coffee")
    ...
    def test_raises_when_the_channel_does_not_exist(
        ...
        with pytest.raises(ChannelNotFoundError):
            GrantChannelManagement(gateway, channels).execute(user_id=7, channel_slug="ghost")
    ...
    def test_records_the_acting_user_in_the_audit_event(
        ...
        with capture_logs() as logs:
            GrantChannelManagement(gateway, channels).execute(
